...
if custom_args.eos_sft:
    train_dataset = prepare_sft_text(
        args, io.load_jsonlines(f"{vars.DATA_DIR}/trivia_qa_wiki_sft/train.jsonl"), tokenizer
    )
else:
    train_dataset = prepare_sft_text(
        args,
        io.load_jsonlines(
            f"{vars.DATA_DIR}/debug_meta_train/syn_data_neurips/model_prep/light_weight_sft_content_curated_v1_sample=10.jsonl"
        ),
        tokenizer,
    )
...
for datum in train_dataset:
    ...
    if all(
        [x in tokenizer(text)["input_ids"] for x in tokenizer(response_template, add_special_tokens=False)["input_ids"]]
    ):
