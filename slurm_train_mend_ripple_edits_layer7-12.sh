#!/bin/bash
#SBATCH -J edit_layer-7-12       # Job name
...
python -m run +alg=mend +experiment=ripple_edits_all +model=llama3.2-1B-eos-sft-7-12 val_steps=100 log_interval=10 val_interval=100 early_stop_patience=2000 +train_set_size=${train_set_size} heavy_outerloop=True mend.shared=False all_propagation_in_outerloop=True
