...
if custom_cfg.syn_data == "drop":
    train_dataset = prepare_sft_text(
        args, custom_cfg, io.load_jsonlines(f"{vars.DATA_DIR}/drop_dataset_converted/drop_dataset_train.jsonl"), tokenizer
    )
    valid_dataset = prepare_sft_text(
        args, custom_cfg, io.load_jsonlines(f"{vars.DATA_DIR}/drop_dataset_converted/drop_dataset_dev.jsonl"), tokenizer
    )
else:
