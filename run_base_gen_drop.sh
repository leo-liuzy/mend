...
# base_model_name=llama3.2-1B-eos-sft
# sft(q_p, a_p)
python run_base_generate_drop.py +alg=mend +experiment=${task} +model=${base_model_name} archive=${archive} eval_only=True generation.save_dir=drop_exp_output/${base_model_name}/drop val_steps=${n_val} edit_loss=sft edit_input=question generation.prompt=${prompt} +do_generation=True +add_eos=True +gen_w_bos=True +add_icl=False
