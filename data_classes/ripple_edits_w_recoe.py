...
class RippleEditsPlusReCoEDataset(Dataset):
    """
    ...
    def __getitem__(self, item, seed=None):
        ...
        if any(e in datum
            ...
            answers = [("" if ans_[0] == " " else " ") + ans_ for ans_ in [datum["answer"]]]
            questions = [q_ + ans_ for q_, ans_ in zip([datum["question"]], answers)]
            output = {
                "texts": deepcopy([datum["texts"]]),
                "recoe_questions": questions,
                "recoe_answers": answers,
            }
        return output
    ...
    def collate_fn(self, batch):
        ...
        """
        if "recoe_questions" in batch[0]:
            all_input_from_batchs["recoe_questions"] = [s for b in batch for s in b["recoe_questions"]]
            all_input_from_batchs["recoe_answers"] = [s for b in batch for s in b["recoe_answers"]]
        else:
    ...
    def edit_generator(self, batch_size, n=None):
        ...
        while True:
            ...
            elif "recoe_questions" in toks["raw"][0]:
                # ekp instances
                edit_outer["input_ids"] = toks["recoe_questions_input_ids"]
                edit_outer["attention_mask"] = toks["recoe_questions_attention_mask"]
                edit_outer["labels"] = self.get_edit_labels(toks["recoe_answers_input_ids"])
            else:
