...
class DropDataset(Dataset):
    ...
    def __init__(
        ...
        assert self.config.heavy_outerloop, "Drop dataset must be used with heavy outerloop."
        
        assert self.config.data.rephrase, "propogation question must be used."
        self.max_length = max_length
        assert self.config.data.zsre_nq, "zsre_nq must be set to True."
    ...
    def __getitem__(self, item, seed=None):
        ...
        assert all(e in self.data[item] for e in ["passage", "qa_pairs"])
        texts = deepcopy([self.data[item]["passage"]])
        
        assert self.config.heavy_outerloop, "Drop dataset must be used with heavy outerloop."
        
        qas = deepcopy(self.data[item]["qa_pairs"])
