...
class RippleEditsDataset(Dataset):
    ...
    def __getitem__(self, item, seed=None):
        ...
        assert all(e in self.data[item] for e in ["edit", "Logical_Generalization", "Compositionality_I", "Compositionality_II", "Subject_Aliasing", "Relation_Specificity", "Forgetfulness"])
        ...
        locality_instances = deepcopy(
            self.data[item]["Relation_Specificity"] + 
            self.data[item]["Forgetfulness"]
        )
        ...
        if len(outerloop_queries) > 0:
            ...
            outer_answers_candidates = [[a["value"] for a in q["answers"] if len(a["value"].strip()) > 0] for q in outerloop_queries]
        ...
        if len(locality_queries) > 0:
            ...
            loc_answers_candidates = [[a["value"] for a in q["answers"] if len(a["value"].strip()) > 0] for q in locality_queries]
    ...
    def edit_generator(self, batch_size, n=None):
        ...
        while True:
            ...
            if "loc_questions" in toks["raw"][0]:
                ripple_edits_loc_questions = [loc_q for ins in toks["raw"] for loc_q in ins["loc_questions"]]
                ripple_edits_loc_answers = [loc_a for ins in toks["raw"] for loc_a in ins["loc_answers"]]
                all_loc_questions += ripple_edits_loc_questions
                all_loc_answers += ripple_edits_loc_answers
            ...
            loc = dict(self.tok(all_loc_questions, return_tensors="pt", padding=True, max_length=self.max_length, truncation=True))
            trg_toks = dict(self.tok(all_loc_answers, return_tensors="pt", padding=True, max_length=self.max_length, truncation=True, add_special_tokens=False))
