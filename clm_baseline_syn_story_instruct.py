...
def generate(
    ...
    formatted_input_str = tokenizer.apply_chat_template([{"role": "user", "content": context}], tokenize=False, add_generation_prompt=True)
    inputs = tokenizer([formatted_input_str], return_tensors="pt", padding=True, add_special_tokens=config.add_bos)
