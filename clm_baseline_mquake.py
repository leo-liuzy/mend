...
def prepare_clm_text(args, custom_cfg, instance, tokenizer):
    ...
    dataset = [f"{rr['context']} {rr['target_new_str']}" for rr in instance["requested_rewrite"]]
...
exp_save_dir = f"{os.getcwd()}/mquake_exp_output/{custom_cfg.base_model_name}_clm-baseline_lr={args.learning_rate}_epoch={args.num_train_epochs}{'_' + custom_cfg.save_dir_suffix if custom_cfg.save_dir_suffix is not None else ''}_tuned-params={custom_cfg.tunable_params}"

os.makedirs(exp_save_dir, exist_ok=True)


if custom_cfg.date_data == "T":
    individual_result_save_dir = f"{exp_save_dir}/individual_results_T"
    cpt_dev_dataset = io.load_jsonlines(f"{vars.DATA_DIR}/mquake_remastered/T/test.jsonl")
elif custom_cfg.date_data == "CF3k":
    individual_result_save_dir = f"{exp_save_dir}/individual_results_CF3k"
    cpt_dev_dataset = io.load_jsonlines(f"{vars.DATA_DIR}/mquake_remastered/CF3k/test.jsonl")
else:
...
question_types = [("efficacy", test_queries)]
