...
def run(config):
    ...
    if config.date_data == "T":
        question_type = "efficacy"
        val_data = io.load_jsonlines(f"{vars.DATA_DIR}/mquake_remastered/T/test.jsonl")
    elif config.date_data == "CF3k":
        question_type = "efficacy"
        val_data = io.load_jsonlines(f"{vars.DATA_DIR}/mquake_remastered/CF3k/test.jsonl")
