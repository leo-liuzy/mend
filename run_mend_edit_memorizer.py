...
def run(config):
    ...
    # for i in tqdm(range(1), desc=f"Running eval on {config.task}"):
        ...
        test_queries = datum["multi_hop_efficacy"] # datum["single_hop_efficacy"]
    ...
    if config.generation.save_dir:
        ...
        all_results.to_excel(
            f"{save_dir}/mend_eval_loss={config.edit_loss}_input={config.edit_input}_n={config.val_steps}_prompt={config.generation.prompt}_{'w' if config.do_generation else 'wo'}-gen_{'w' if hasattr(config, 'add_icl') and config.add_icl else 'wo'}-icl-sh+mh.xlsx",
            index=False,
        )
        io.dump_jsonlines(
            edit_model_infos,
