...
for date_data in 4K_test_id 4K_test_ood 4K_test_ood-entity 4K_test_ood-relation # 4K_test_id  # 4K_test_ood 4K_test_ood-entity 4K_test_ood-relation  # profiling
do
...
# top + qwen2.5-1.5B
# python run_original_mend_edit_syn_story.py +alg=mend +experiment=${task} +model=qwen2.5-1.5B-eos-sft-template-format-curated-v1-lr2e-6-sample-10-top archive=${archive} eval_only=True generation.save_dir=synstory_exp_output/${exp_dir_name}/${task} val_steps=${n_val} edit_loss=sft edit_input=seen generation.prompt=${prompt} +do_generation=True +add_bos=True +add_eos=True +add_eos_accuracy=True +gen_w_bos=True +add_icl=False +spec_question=True +date_data=${date_data} mend.shared=True
