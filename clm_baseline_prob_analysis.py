...
with torch.no_grad():
    ...
    for question_type in question_types:
        ...
        for q_i, question in enumerate(questions):
            ...
            post_edit_answer_logits = post_edit_logits[:, :-1]
            post_edit_answer_logits = post_edit_answer_logits[:, -sft_labels.size(1) :]
            mask = sft_labels != -100
            n_tokens = mask.float().sum()
            
            unmasked_log_probs = post_edit_answer_logits.log_softmax(-1).gather(-1, sft_labels.unsqueeze(-1)).squeeze(-1)
            
            df_content.append(
                {
                    "stage": "pre-edit",
                    "question_tag": question_type + f"q{q_i}",
                    "question_type": question_type,
                    "id": eval_instance["id"],
                    "question": test_queries_q_str,
                    "answer": test_queries_a_str,
                    "answer_ids": sft_labels.tolist()[0],
                    "n_tokens": n_tokens.cpu().item(),
                    "log_prob": unmasked_log_probs.tolist()[0]
                }
            )
    ...
    with torch.no_grad():
        ...
        for question_type in question_types:
            ...
            for q_i, question in enumerate(questions):
                ...
                post_edit_output = unwrapped_model(
                    input_ids=acc_toks["input_ids"],
                    attention_mask=acc_toks["attention_mask"]
                )
                post_edit_logits = post_edit_output.logits
...
individual_result_save_dir = f"{exp_save_dir}/prob_analysis"
os.makedirs(individual_result_save_dir, exist_ok=True)

fpath = f"{individual_result_save_dir}/{instance['id']}_prob_info.json"
io.dump_jsonlines(df_content, fpath)
