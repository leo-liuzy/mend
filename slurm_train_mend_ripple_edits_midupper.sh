#!/bin/bash
#SBATCH -J share_midupper       # Job name
...
python -m run +alg=mend +experiment=ripple_edits_all +model=llama3.2-1B-eos-sft-mid-upper  val_steps=100 log_interval=10 val_interval=100 early_stop_patience=2000 +train_set_size=${train_set_size} heavy_outerloop=True mend.shared=True all_propagation_in_outerloop=True
