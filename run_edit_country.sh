export CUDA_VISIBLE_DEVICES=2025-03-20_20-29-05_6346534455
...
do
python run_mend_edit_country.py +alg=mend +experiment=${task} +model=llama3.2-1B-common- archive=${archive} eval_only=True generation.save_dir=debug_exp_output/${exp_dir_name}/${task} val_steps=${n_val} edit_loss=clm edit_input=seen generation.prompt=${prompt} +do_generation=True +add_bos=True +add_eos=True +add_eos_accuracy=True +gen_w_bos=True +add_icl=False +spec_question=True +date_data=${date_data}
done
