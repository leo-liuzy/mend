...
exp_save_dir = f"{os.getcwd()}/drop_exp_output/{custom_cfg.base_model_name}_clm-baseline_lr={args.learning_rate}_epoch={args.num_train_epochs}{'_' + custom_cfg.save_dir_suffix if custom_cfg.save_dir_suffix is not None else ''}"

os.makedirs(exp_save_dir, exist_ok=True)


individual_result_save_dir = f"{exp_save_dir}/individual_results"
cpt_dev_dataset = io.load_jsonlines(f"{vars.DATA_DIR}/drop_dataset_converted/drop_dataset_dev.jsonl")

os.makedirs(individual_result_save_dir, exist_ok=True)

assert not custom_cfg.spec_question
...
logging.info(f"Example: {instance['section_id']}")

fpath = (
    f"{individual_result_save_dir}/{instance['section_id']}_eval_results"
...
for question_type, questions in question_types:
    ...
    for q_i, question in tqdm(enumerate(questions), total=len(questions)):
        ...
        post_result_df.insert(0, "id", instance['section_id'])
