...
def get_eval_result(question, answer, model, tokenizer, config, generation_config):
    ...
    acc_toks = add_eos(
        tokenizer(test_queries_str, padding=True, return_tensors="pt", add_special_tokens=config.add_bos),
        eos_token_id,
        ignore=not config.add_eos_accuracy,
    )
    ...
    with torch.no_grad():
        model_output = model(input_ids=acc_toks["input_ids"], attention_mask=acc_toks["attention_mask"])
...
def get_analysis_result(question, answer, model, tokenizer, config, generation_config):
    test_queries_str = [question + (" " if answer[0] != " " else "") + answer]
