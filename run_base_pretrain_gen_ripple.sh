...
base_model_name=llama3.2-1B-eos-sft-ripple_edits-pretrain-all
# base_model_name=llama3.2-1B-eos-sft-ripple_edits-pretrain-top3
# base_model_name=llama3.2-1B-eos-sft-ripple_edits-pretrain-midupper3

# base_model_name=llama3.2-1B-eos-sft
# sft(q_p, a_p)
python run_base_pretrain_generate_ripple.py +alg=mend +experiment=${task} +model=${base_model_name} archive=${archive} eval_only=True generation.save_dir=ripple_exp_output/${base_model_name} val_steps=${n_val} edit_loss=sft edit_input=question generation.prompt=${prompt} +do_generation=True +add_eos=True +gen_w_bos=True +add_icl=False +ice=True +date_data=all_propagation