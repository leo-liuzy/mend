...
python run_base_generate_mquake.py +alg=mend +experiment=${task} +model=${base_model_name} archive=${archive} eval_only=True generation.save_dir=mquake_exp_output/${base_model_name} val_steps=${n_val} edit_loss=clm edit_input=question generation.prompt=${prompt} +do_generation=True +add_eos=True +gen_w_bos=True +add_icl=False +ice=False +date_data=CF3k